...
В дополнительном коде можно хранить числа от `-2**n` до `2**n-1` где n количество доступных битов для хранения чиселки.  


Итак, в итоге:   
```
Чтобы перевести число в дополнительный код, нужно взять его обратный код и добавить один  
Чтобы из дополнительного кода перевести число в прямой, нужно взять его дополнительный код, инвертировать и тоже добавить один. 
```

На примере того, что обсуждали на семинаре:
```
-8 = 0b1000 -> (инвертируем) -> 0b0111 -> (добавляем один) -> 0b1000
Обратно 
0b1000 -> инвертируем -> 0b0111 -> добавляем один -> 0b1000 -> получаем -8  (изначально код дополнительный, первый бит был единичкой, значит число отрицательное).
```
...
[Статья](https://ryanstutorials.net/binary-tutorial/binary-floating-point.php) в которой есть прикольное объяснения и интерактивная тыкалка с процессом перевода.  
...
## Примеры задач с контрольных на эту тему

- Чему равно вещественное значение, закодированного в формате IEEE754 с одинарной точностью `0x40C00000`
- Чему равна сумма двух вещественных чисел в IEEE754 c одинарной точностью: `0x40C00000`, `0xFF800000`
- Чему равно десятичное представление знакового целого 16-битного числа `0xACF5`